...
def parse_file(filename: str):
    with open(filename) as f:
        for lines in f.read().split("\n\n"):
            yield map(int, lines.split())


def part1(filename: str):
    return max(sum(calories) for calories in parse_file(filename))


def part2(filename: str):
    return sum(
        sorted((sum(calories) for calories in parse_file(filename)), reverse=True)[:3]
    )
